export class MoviesStorage {
    ...
    constructor() {
        this.storage = JSON.parse(localStorage.getItem('movies'))
        if (this.storage === undefined || !(Array.isArray(this.storage))) {
    ...
    set(data, id) {
        ...
        if (data !== undefined && data !== null) {
            if (id !== undefined) {
                ...
                movieIndex = this.storage.indexOf(movieToEdit)
                this.storage[movieIndex] = data
            } else {
                this.storage.push(data)
            }
            localStorage.setItem('movies', JSON.stringify(this.storage))
        }
    }
    ...
    remove(id) {
        ...
        movieIndex = this.storage.indexOf(movieToEdit)
        this.storage.splice(movieIndex, 1)
        localStorage.setItem('movies', JSON.stringify(this.storage))
    }
